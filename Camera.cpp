...
glm::vec3 Camera::returnPixel(Ray *r, Triangle *T, int nrbounces) {
		...
		> glm::distance(r->getRayorigin(), instersectionPointSphere))
	{
		...
		normal = glm::vec3(2 * instersectionPointSphere.x, 2 * instersectionPointSphere.y, 2 * instersectionPointSphere.z);

		newOrigin = instersectionPointSphere+ 0.03f*(normal + instersectionPointTriangle - r->getRayorigin());
	}
	else {
		...
		newOrigin = instersectionPointTriangle + 2.f*(normal);
	...
	//set shadowray direction and origin
	r->setRayOrigin(newOrigin);
	r->setRayDirection((L->getLightPosition() - newOrigin));

	//std::cout << r->getDirection().x << " : " << r->getDirection().y << " : " << r->getDirection().z << std::endl;
	//check shadowray intersection
	T->molllerTrombore(T->getTriangles(), r, instersectionPointTriangle, pixelColor2, t);
	T->sphereIntersect(T->getSpheres(), r, instersectionPointSphere, pixelColor2);
	...
	//r->setRayDirection(D->calculateBounce(T, r, newOrigin, id, identn normal));
